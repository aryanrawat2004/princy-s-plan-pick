...
import {
  ...
  IceCreamBowl, MapPin, PartyPopper, Pizza, RotateCcw, Sparkles, Star, SunMedium,
} from "lucide-react";
...
export function PlanExperience() {
  ...
  const progress = useMemo(() => Math.min(100, Math.max(0, ((state.step - 1) / 9) * 100)), [state.step]);
  ...
  const afterActivity = () => set({ step: preferenceNeeded ? 4 : 5 });
  ...
  const confirm = async () => {
    ...
    toast.success("Plan saved ✨"); set({ confirmed: true, step: 10 });
  };
    ...
    <main className="relative min-h-dvh overflow-hidden bg-background px-4 py-5 sm:grid sm:place-items-center sm:py-10">
      ...
      <section className="relative z-10 mx-auto flex min-h-[calc(100dvh-2.5rem)] w-full max-w-[460px] flex-col overflow-hidden rounded-[28px] border border-border/80 bg-card/80 shadow-shell backdrop-blur-xl sm:min-h-[780px] sm:max-h-[900px]">
        <header className="grid grid-cols-[44px_minmax(0,1fr)_44px] items-center gap-2 px-4 pt-4">
          <Button aria-label="Go back" title="Go back" variant="ghost" size="icon" className={cn("rounded-full", (state.step === 0 || state.step === 10) && "invisible")} onClick={back}><ArrowLeft /></Button>
          <div className="min-w-0 text-center">
            {state.step > 0 && state.step < 10 && <><p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-muted-foreground">Step {Math.min(state.step, 9)} of 9</p><div className="mx-auto mt-2 h-1 w-full max-w-48 overflow-hidden rounded-full bg-muted"><motion.div className="h-full bg-primary" animate={{ width: `${progress}%` }} /></div></>}
          </div>
