...
function AdminPage() {
  ...
  const copy = async (plan: Plan) => {
    await navigator.clipboard.writeText(`${plan.activity} • ${plan.selected_date} • ${plan.selected_time} • ${plan.location_preference}${plan.activity_preference ? ` • ${plan.activity_preference}` : ""}${plan.note ? `\nNote from Princy: ${plan.note}` : ""}`);
